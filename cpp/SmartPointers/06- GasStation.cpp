...
class FuelCan : public GasolineSource {
 public:
  void FuelUp() { std::cout << "Pumping gas from fuel can" << std::endl; }
};
...
int main() {
  std::shared_ptr<GasolineSource> stationService =
      std::make_shared<GasolineSource>(new GasStation());

  // GasolineSource *stationService = new GasStation();

  std::shared_ptr<GasolineSource> canService =
      std::make_shared<GasolineSource>(new FuelCan());
